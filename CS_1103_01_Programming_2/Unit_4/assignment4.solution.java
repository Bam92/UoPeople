/**
 Problem #1 :  import java.util.Arrays; instead of import java.util.*;
    ...
    Problem #2: In "contains" method should remove the "else" part 
otherwise it will exist on the first number which is not equal to 5.

    Problem #3 : In "bubbleSort" method , In the second loop it should be 
"j++" instead of "i++".

    Problem #4: In "selectionSort" method, In the first "if" statement it 
should be "I" instead of "1".

    Problem #5 : In "insertionSort" method , In the while it should be 
"pos >=0" instead of "pos>0" .

    Problem #6 : In "insertionSort" method , In the last line , should be 
"array[pos+1]" instead of "array[pos]".
...
public class BuggySearchAndSort {
 ...
 public static void main(String[] args) {
  ...
  int[] A = new int[10];  // Create an array and fill it with small random 
ints.
  for (int i = 0; i < 10; i++)
  ...
  * method simply sweeps through the array, exchanging neighboring 
elements
 ...
 /**
  * Sorts an array into non-decreasing order.  This method uses a 
selection
  * sort algorithm, in which the largest item is found and placed at the 
end of
  * the list, then the second-largest in the next to last place, and so 
on.
  */
